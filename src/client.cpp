...
int connect_to_manager(const char* ip, int port) {
    ...
    if (sock < 0) return socket_error("[CLIENT] Erro ao criar socket.", sock);
...
void request_sensor_data(int socket_fd, uint8_t sensor_id) {
    ...
    // Tudo certo?
    if (ack_header.msg_type == static_cast<uint8_t>(MessageType::CLIENT_GET_ACK)) {
        float reading;
        if (recv(socket_fd, &reading, sizeof(reading), 0) <= 0) {
            cerr << "[CLIENT] Erro ao ler payload da leitura.\n";
            return;
        }
        cout << ">>> [CLIENT] Leitura do Sensor ID " << (int)sensor_id << ": " << reading << "\n";
    } else {
        cerr << "[CLIENT] Erro: Resposta inesperada do Gerenciador (Opcode: " << (int)ack_header.msg_type << ").\n";
    }
}

void set_sensor_threshold(int socket_fd, uint8_t sensor_id, float min_val, float max_val) {
    GCPHeader header = create_header(MessageType::CLIENT_SET_THRESHOLD, sensor_id);
    ...
    if (ack_header.msg_type == static_cast<uint8_t>(MessageType::CLIENT_SET_THRESHOLD_ACK)) {
        uint8_t status;
        ...
        if (status == 0x00) {
            cout << ">>> [CLIENT] Limites do Sensor ID " << (int)sensor_id << " atualizados!\n";
        } else {
            cout << ">>> [CLIENT] Falha ao configurar limites (Valores invalidos/recusados pelo servidor).\n";
        }
    } else {
        cerr << "[CLIENT] Erro: Mensagem inesperada do Gerenciador (Opcode: " << (int)ack_header.msg_type << ").\n";
    }
}
